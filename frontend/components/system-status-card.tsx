...
function serviceModelBadges(name: string, service: ServiceStatus) {
  ...
  const cloudModels = service.cloud_models ?? [];
  const configuredCloud = service.configured_cloud_models ?? [];
  return (
    <div className="mt-2 flex flex-wrap gap-2">
      ...
      {configuredCloud.length ? (
        <Badge
          variant="outline"
          className="border-destructive/40 text-destructive"
          title={configuredCloud.join(", ")}
        >
          configured cloud model: {configuredCloud.join(", ")}
        </Badge>
      ) : cloudModels.length ? (
        <Badge variant="outline" title={cloudModels.join(", ")}>
          cloud entries listed: {cloudModels.length} (not used)
        </Badge>
      ) : (
        <Badge variant="secondary">no cloud entries</Badge>
      )}
    </div>
  );
}
...
export function SystemStatusCard() {
  ...
  return (
    <Card>
      ...
      {/* minmax(0, 1fr) column keeps long service errors inside narrow cards */}
