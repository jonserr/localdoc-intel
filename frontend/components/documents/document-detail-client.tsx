...
export function DocumentDetailClient({ id }: { id: string }) {
  ...
  const summary: [string, string | undefined][] = [
    ...
    ["Text ingestion", document?.status],
    ["Vector index", document?.vector_indexing ?? "not_requested"],
    ["Ingested", formatDate(document?.last_indexed_at)],
  ];
  const parserRows: [string, unknown][] = [
    ...
    ["Vector index error", metadata.vector_index_error],
    ["Vector index attempts", metadata.vector_indexing_attempts],
      ...
      <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={loading || reindexing || !document}
          >
