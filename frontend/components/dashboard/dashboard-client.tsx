...
import { VectorIndexBadge } from "@/components/documents/vector-index-badge";
...
type DashboardState = {
  ...
  // Totals before the recent-item lists are truncated
  queryCount: number;
...
export function DashboardClient() {
  ...
  const indexingStates: VectorIndexingState[] = [
    "queued",
    "running",
    "succeeded",
    "failed",
  ];
  const vectorCounts = data.stats?.documents_by_vector_indexing;
  const cards = useMemo(
    () => [
      {
        label: "Documents",
      ...
      },
      {
        ...
        detail: "saved questions and answers",
        icon: MessageSquareText,
      },
      {
        label: "Evaluation runs",
        value: formatNumber(data.evaluationCount),
        detail: !latestEvaluation
          ? "no runs yet"
          : latestEvaluation.judged_answer_count > 0
            ? `${formatPercent(latestEvaluation.answer_quality_score)} judge score (latest run)`
            : "latest run has no judge score",
        icon: BarChart3,
      },
    ],
    [data.evaluationCount, data.queryCount, data.stats, latestEvaluation],
  );
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Embedding progress is separate from text ingestion. Open a document
            to review or retry a failed index.
          </CardDescription>
        </CardHeader>
          ...
          {loading ? (
            ...
            indexingStates.map((state) => (
              <VectorIndexBadge
                key={state}
                state={state}
                count={vectorCounts[state] ?? 0}
              />
            ))
          ) : (
            <p className="text-sm text-muted-foreground">
              Vector indexing counts are not available.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
        <SystemStatusCard />
        <Card>
          <CardHeader>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
        <Card>
          <CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Text ingestion</TableHead>
                ...
                <TableBody>
                  {data.documents.map((document) => (
                    <TableRow key={document.id}>
                      ...
                      <TableCell>
                        <VectorIndexBadge state={document.vector_indexing} />
                      </TableCell>
                      <TableCell>
