...
def fake_metrics(questions, **kwargs) -> EvaluationMetrics:
    """Stand in for a real run. Built from the dataclass so fields cannot drift."""
    return EvaluationMetrics(
        ...
        effective_config={"vector_min_score": 0.0},
    )
...
def test_run_evaluation_computes_metrics(indexed_documents):
    ...
    assert metrics.expected_term_coverage == pytest.approx(2 / 3)
...
def test_run_eval_command_creates_run(indexed_documents, tmp_path, monkeypatch):
    ...
    call_command(
        "run_eval", "--questions", str(questions_path), "--name", "Command run"
    )

    run = EvaluationRun.objects.get(name="Command run")
...
def test_run_eval_command_always_judges_answers(
    ...
    def capturing_run_evaluation(questions, **kwargs):
        captured.update(kwargs)
        return fake_metrics(questions, **kwargs)

    monkeypatch.setattr(
        "evaluations.management.commands.run_eval.run_evaluation",
        capturing_run_evaluation,
    )
    questions_path = tmp_path / "questions.json"
    questions_path.write_text(
        json.dumps(
    ...
    run = EvaluationRun.objects.get(name="Judged command run")
