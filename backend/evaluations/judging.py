...
def judge_answer_quality(
    *,
    ...
    # Normalization runs inside the guard so a malformed verdict is recorded as
    # unjudged with an error instead of ending the evaluation run.
    try:
...
def normalize_answer_quality_result(raw_result) -> AnswerQualityResult:
    ...
    # bool subclasses int, so True would otherwise clamp to a perfect score.
    if isinstance(raw_score, bool):
        raise AnswerJudgeError("Answer judge score must be a number, not a boolean.")
    try:
    ...
    # NaN and the infinities survive clamping: min(1.0, nan) returns 1.0.
    if not math.isfinite(score):
        raise AnswerJudgeError("Answer judge score must be a finite number.")
