...
CPP_CHUNK_RECORD = struct.Struct("<IIIQQ")
# Conservative cutoff above the benchmark crossover. Recorded gains at 4 MiB
# varied between runs. See the historical table in cpp/chunker/README.md.
CPP_CHUNKER_THRESHOLD_BYTES = 4 * 1024 * 1024
...
def chunk_parsed_sources(
    ...
    for source in parsed_sources:
        ...
        if (
            ...
            and len(source_bytes) > CPP_CHUNKER_THRESHOLD_BYTES
        ):
...
def find_cpp_chunker() -> Path | None:
    ...
    candidates = [
        ...
        # Installed by the backend Docker image, outside development bind mounts.
...
def run_cpp_chunker(
    ...
    try:
        content = source_bytes if source_bytes is not None else input_path.read_bytes()
    ...
    if not output or len(output) % CPP_CHUNK_RECORD.size:
        return None

    chunks: list[ChunkPayload] = []
    try:
        for expected_index, record in enumerate(CPP_CHUNK_RECORD.iter_unpack(output)):
            chunk_index, start_line, end_line, byte_start, byte_end = record
            if (
            ...
            # The C++ offsets address raw bytes. Normalize only line separators
            # while reconstructing the same text produced by _chunk_text.
            text = (
            ...
            chunks.append(
                ChunkPayload(
                    ...
                    text=text,
                    page=None,
                    start_line=_nullable_positive_int(start_line),
                    end_line=_nullable_positive_int(end_line),
                    byte_start=_nullable_non_negative_int(byte_start),
                    byte_end=_nullable_non_negative_int(byte_end),
...
# Line semantics shared by the Python and C++ chunkers: a line ends at "\n",
# "\r\n", or a lone "\r". Other characters that str.splitlines() treats as
# breaks (form feed, vertical tab, NEL, U+2028/U+2029) stay inside the line so
# that both chunkers agree and every separator has a known byte width.
LINE_BREAK = re.compile(r"\r\n|\r|\n")


def _line_records(text: str) -> list[dict]:
    """Split ``text`` into lines with byte offsets into its UTF-8 encoding.

    Offsets account for the actual separator width (1 byte for "\n"/"\r",
    2 bytes for "\r\n"). A trailing separator does not start an empty line.
    """
    records = []
    byte_offset = 0
    position = 0
    ...
    for match in LINE_BREAK.finditer(text):
        line = text[position : match.start()]
        ...
        records.append(
            {
                ...
                "byte_end": byte_offset + encoded_length,
            }
        )
        byte_offset += encoded_length + len(match.group().encode("utf-8"))
        position = match.end()
        line_number += 1
...
def ingest_bytes(
    *,
    ...
    null_bytes_removed = any("\x00" in source.text for source in parsed_sources)
    ...
    # Byte offsets address the original file only when the stored text is the
    # file's own UTF-8 bytes: text/code files that decoded strictly and had no
    # NUL bytes removed. Every other format yields offsets into the extracted
    # (transformed) text. Only raw sources are eligible for the C++ chunker,
    # which reads the file bytes directly.
    raw_source_text = (
        extension in TEXT_EXTENSIONS | CODE_EXTENSIONS
        and not null_bytes_removed
        and not any(source.metadata.get("decode_errors") for source in parsed_sources)
    )
    chunks, chunker = chunk_parsed_sources(
        parsed_sources,
        chunk_size=chunk_size,
        overlap=overlap,
        source_file_path=source_file_path if raw_source_text else None,
        source_bytes=content if raw_source_text else None,
    )
    if raw_source_text:
        offset_basis = "source_bytes"
    elif any(chunk.byte_start is not None for chunk in chunks):
    ...
    # Slow parsing, OCR, conversion and chunking are complete before opening
    # the write transaction. Keep document/chunk replacement all-or-nothing.
...
VECTOR_INDEXING_STATES = ("disabled", "queued", "running", "succeeded", "failed")


def set_vector_indexing_state(document: Document, state: str, **fields) -> None:
    """Persist the vector-indexing lifecycle state on the document metadata.

    Text ingestion (``Document.status``) and vector indexing are separate
    outcomes: a document is keyword-searchable as soon as its chunks are
    stored, while ``metadata["vector_indexing"]`` tracks the vector index.
    """
    if state not in VECTOR_INDEXING_STATES:
        raise ValueError(f"Unknown vector indexing state '{state}'.")
    metadata = {**document.metadata, "vector_indexing": state, **fields}
    Document.objects.filter(id=document.id).update(metadata=metadata)
    document.metadata = metadata


def apply_indexing_result(
    document: Document,
    indexing_result,
    *,
    attempts: int = 1,
    ...
    """Record a vector indexing outcome on the document's metadata.

    A failed attempt that Celery will retry goes back to ``queued`` and keeps
    the error visible. A final failure is ``failed``.
    """
    if not indexing_result.enabled:
        set_vector_indexing_state(document, "disabled")
        return
    if indexing_result.error:
        set_vector_indexing_state(
            document,
            "queued" if retry_pending else "failed",
            vector_indexed=0,
            vector_index_error=indexing_result.error,
            vector_indexing_attempts=attempts,
        )
        return
    set_vector_indexing_state(
        document,
        ...
        vector_index_error="",
        vector_indexing_attempts=attempts,
        vector_indexed_at=timezone.now().isoformat(),
    )
...
def _schedule_indexing(document: Document) -> None:
    ...
    The inline fallback triggers only when dispatch to the broker fails
    (for example Redis is down). A reachable broker with no running worker
    accepts the task, so the document stays ``queued`` until a worker starts
    or the document is reindexed; the status endpoint reports whether a
    worker is consuming.
    """
    ...
    if settings.ASYNC_INDEXING_ENABLED:
        ...
        def dispatch() -> None:
            try:
                ...
                document.metadata = {
                    **document.metadata,
                    "vector_indexing_dispatch_error": f"{type(exc).__name__}: {exc}",
                }
        ...
        set_vector_indexing_state(
            ...
            "queued",
            vector_index_error="",
            vector_indexing_dispatch_error="",
        )
        transaction.on_commit(dispatch)
        return

    # on_commit also protects callers with an enclosing transaction. Outside
    # a transaction it executes immediately, preserving synchronous responses.
    def index_inline() -> None:
        set_vector_indexing_state(document, "running")
        apply_indexing_result(document, index_document_chunks(document.id))

    set_vector_indexing_state(document, "queued", vector_index_error="")
    transaction.on_commit(index_inline)


def reindex_document(document: Document) -> Document:
    """Reset the vector-indexing state and dispatch indexing again.

    This is the recovery path for ``failed`` documents and for ``queued``
    documents whose task was lost (for example a broker restart).
    """
    _schedule_indexing(document)
