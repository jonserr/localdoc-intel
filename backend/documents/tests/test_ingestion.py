...
def test_text_ingestion_uses_cpp_chunker_when_available(monkeypatch):
    ...
    def fake_run(command, **kwargs):
        ...
        return CompletedProcess(
            command,
            0,
            stdout=CPP_CHUNK_RECORD.pack(0, 1, 2, 0, len(content)),
            stderr=b"",
        )
    ...
    assert chunk.byte_end == len(content)
...
@pytest.mark.django_db
@override_settings(MEDIA_ROOT="/tmp/localdoc-test-media")
def test_small_text_ingestion_skips_cpp_chunker(monkeypatch):
    def fail_if_called(*args, **kwargs):
        raise AssertionError("small files must not spawn the C++ chunker")

    monkeypatch.setattr("documents.ingestion.subprocess.run", fail_if_called)

    result = ingest_bytes(
        filename="small.txt",
        content=b"the Python path avoids subprocess overhead\n",
...
def test_text_at_cpp_threshold_stays_on_python(monkeypatch):
    source_bytes = b"x" * CPP_CHUNKER_THRESHOLD_BYTES

    def fail_if_called(*args, **kwargs):
        raise AssertionError("a file exactly at the threshold must stay on Python")
    ...
    chunks, chunker = chunk_parsed_sources(
        ...
        source_file_path=Path("/tmp/threshold.txt"),
        source_bytes=source_bytes,
    )

    assert chunker == "python"
    assert chunks[0].text == source_bytes.decode("utf-8")


def test_cpp_chunker_reconstructs_crlf_text_from_raw_offsets(tmp_path, monkeypatch):
    content = b"alpha\r\nbeta\r\ngamma"
    input_path = tmp_path / "crlf.txt"
    ...
    monkeypatch.setattr(
        "documents.ingestion.subprocess.run",
        lambda command, **kwargs: CompletedProcess(
            command,
            0,
            stdout=CPP_CHUNK_RECORD.pack(0, 1, 2, 0, 11),
            stderr=b"",
        ),
    )

    chunks = run_cpp_chunker(input_path, 0, 1200, 150, source_bytes=content)

    assert chunks is not None
    assert chunks[0].text == "alpha\nbeta"
    assert chunks[0].byte_start == 0
    assert chunks[0].byte_end == 11
    assert content[chunks[0].byte_start : chunks[0].byte_end] == b"alpha\r\nbeta"


def test_cpp_chunker_rejects_malformed_binary_output(tmp_path, monkeypatch):
    input_path = tmp_path / "malformed.txt"
    ...
    monkeypatch.setattr(
        "documents.ingestion.subprocess.run",
        lambda command, **kwargs: CompletedProcess(
            command, 0, stdout=b"not-a-complete-record", stderr=b""
        ),
    )

    assert run_cpp_chunker(input_path, 0, 1200, 150) is None
...
def test_ingest_folder_ingests_markdown_and_code(
    client, tmp_path, settings, ocr_enabled
):
    settings.OCR_ENABLED = ocr_enabled
    ...
    # Use an unsupported archive; PNG is supported and invokes the OCR path.
    (tmp_path / "ignore.zip").write_bytes(b"ignored")
