...
import requests
from config.model_policy import configured_remote_models, remote_model_names
...
def celery_worker_status() -> dict:
    """Report whether a Celery worker is consuming the indexing queue.

    A reachable broker with no worker accepts tasks that nobody runs, so
    ``queued`` documents would wait forever. This probe pings workers with a
    short timeout; it does not start one.
    """
    if not settings.ASYNC_INDEXING_ENABLED:
        return {"available": False, "enabled": False, "workers": []}
    try:
        from config.celery import app

        replies = app.control.inspect(timeout=PROBE_TIMEOUT_SECONDS).ping() or {}
        workers = sorted(replies)
        return {"available": bool(workers), "enabled": True, "workers": workers}
    except Exception as exc:
        return {"available": False, "enabled": True, "workers": [], "error": str(exc)}
...
def ollama_status() -> dict:
    try:
        ...
        pulled = [row.get("name", "") for row in payload.get("models", [])]
        # Cloud-backed entries are listed locally but served by Ollama's cloud.
        remote = remote_model_names(payload)
        configured_remote = configured_remote_models(
            {
        ...
        return {
            ...
            "cloud_models": remote,
            "configured_cloud_models": sorted(set(configured_remote.values())),
        }
    except Exception as exc:
        return {
            "available": False,
