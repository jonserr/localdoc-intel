...
from .ingestion import IngestionError, ingest_bytes, ingest_folder, reindex_document
...
def stats(request):
    ...
    documents_by_vector_indexing = {
        state: Document.objects.filter(metadata__vector_indexing=state).count()
    ...
    return Response(
        {
            ...
            "documents_by_vector_indexing": documents_by_vector_indexing,
        }
    )
    ...
    return Response(
        {
            ...
            "privacy": (
                "Documents are processed by the configured services. The "
                "defaults point every service at this machine; changing a "
                "service URL changes where documents go."
            ),
...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def reindex(self, request, pk=None):
        """Re-dispatch vector indexing for a failed or stuck document."""
